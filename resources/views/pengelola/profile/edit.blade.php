@extends('layouts.main_pengelola') <!-- Pastikan menggunakan layout pengelola -->
...
<div class="container mx-auto mt-8 px-6">
    ...
    <div class="bg-white p-6 rounded-lg shadow-md flex items-center space-x-6">
        <img id="profile-picture-preview"
            src="{{ $pengelola->profile_picture ? asset('storage/' . $pengelola->profile_picture) : 'https://via.placeholder.com/150' }}"
            alt="Profile Picture" class="w-32 h-32 rounded-full object-cover border-4 border-blue-500 shadow-lg" />
        <div>
            <h2 class="text-3xl font-semibold text-gray-700">{{ $pengelola->name }}</h2>
            <p class="text-gray-500">Role: Pengelola</p>
            <p class="text-gray-500">Terdaftar sejak: {{ $pengelola->created_at->format('d F Y') }}</p>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md mt-6">
        ...
        <form action="{{ route('pengelola.profile.update') }}" method="POST" enctype="multipart/form-data"
            ...
            <div>
                ...
                <input type="text" id="name" name="name" value="{{ old('name', $pengelola->name) }}"
            ...
            <div>
                ...
                <input type="email" id="email" name="email" value="{{ old('email', $pengelola->email) }}"
                    class="w-full border-gray-300 rounded-lg shadow-sm p-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            ...
            <div>
                ...
                <input type="text" id="phone_number" name="phone_number"
                    value="{{ old('phone_number', $pengelola->phone_number) }}"
                    class="w-full border-gray-300 rounded-lg shadow-sm p-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            ...
            <div>
                ...
                <select id="region_id" name="region_id"
                    ...
                    <option value="{{ $region->id }}" {{ old('region_id', $pengelola->region_id) == $region->id ? 'selected'
                        : '' }}>
                        {{ $region->name }}
                    </option>
                    @endforeach
